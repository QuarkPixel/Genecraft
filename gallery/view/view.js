...
//Eggs
if (arrNumber * 0.02 > poundSign && poundSign != 0) {
    if (window.localStorage.getItem("egg")) {
        ...
        if (times) {
            ...
            if (times >= 3 && times < 6) {
                $("body").append(
                    '<div id="iqrs">别再刷新了，彩蛋就只触发一次(*￣0￣)</div>'
                )
            } else if (times >= 6 && times < 10) {
                $("body").append(
                    '<div id="iqrs">行啦，就给你<a class="href" href="javascript:window.localStorage.clear();window.sessionStorage.clear();console.log(' +
                        "'clear'" +
                        ');location.reload();">再来一次</a>吧</div>'
                )
            }
            $("#iqrs").css({
                display: "inline",
        ...
        $("body").append(
            '<a href="/surprise/index.html#7a1t2dxh4h0u" target="_blank" id="iqrs">彦祖&emsp;你来考古辣~</a>'
        )
        $("#iqrs").css({
            display: "inline",
            position: "absolute",
...
displayImg.addEventListener("click", function () {
    if (zoom) {
        ...
        $("#displayShadow").css("transform", "scale(1)")
        ...
        zoomRatio *= 1.2
        $("#displayShadow").css("transform", "scale(" + zoomRatio + ")")
    }
})
