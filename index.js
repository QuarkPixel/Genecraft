...
if (numberRows != 0) {
    var interval = Math.round(20000 / (numberRows + .6));
    var rowClearance = (boxH - 50 * numberRows) / (2 * numberRows);
    var rowPosition = new Array();
    for (i = 0; i < numberRows; i++) {
        rowPosition.push((2 * i * rowClearance + rowClearance + 50 * i).toFixed());
    };
    randomArray_Times = 0;
    randomArrayFunction();
    ...
    setInterval(function () {
        ...
        if (Math.random() * 10 < .2) {
            var newText = $('<a href="/surprise/index.html#hgg1mlsdlpiy" target="_blank">真的有人会看这个嘛 awa</a>');
        } else {
...
for (let i = 0; i < 7; i++) {
    $('.item').append('<div><a href="gallery" class="imgA"><img src="' + imgHref[randomArrayImages[i]].replace("&r;", abbreviations) + '"></a></div>');
}
